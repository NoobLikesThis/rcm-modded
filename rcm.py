...
class RobloxFileReplacer(tk.Tk):
    ...
    def create_cache(self):
        ...
        combo = ttk.Combobox(dialog, values=[d for _,d in self.functions], state='readonly')
        combo.pack(padx=10, pady=5); combo.current(0)
        ttk.Label(dialog, text="Enter display name:").pack(padx=10, pady=(10,0))
        entry = ttk.Entry(dialog)
        entry.pack(padx=10, pady=5)
        def on_ok():
            func = combo.get(); disp = entry.get().strip(); dialog.destroy()
            key = next((k for k,d in self.functions if d==func), None)
            if not key or not disp:
                messagebox.showerror("Error","Function and name required.")
                return
            name = f"{key} - {disp}"
